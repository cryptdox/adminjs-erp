...
import { useNewSaleOrder } from "./hooks/new-sale-order.hook.js";

type Variant = { id: string; name: string };

const NewSaleOrder = (props: BasePropertyProps) => {
  const {
    orderNumber,
    selectedSupplier,
    setSelectedSupplier,
  ...
  return (
    <div className="!w-full !p-6 !bg-white !rounded-md !shadow-md">
      ...
      <div className="!grid !grid-cols-1 sm:!grid-cols-2 !gap-4 !mb-6">
        ...
        <div>
          <Label className="!text-sm !font-semibold !text-gray-500 !mb-1 !block">Customer</Label>
          <Select
            value={selectedSupplier || ""}
            onChange={(selected) => setSelectedSupplier(selected || "")}
            options={partners
              .filter((sup) => sup.type == 'CUSTOMER')
              .map((sup) => ({
                value: sup.id,
                label: sup.name,
              }))}
